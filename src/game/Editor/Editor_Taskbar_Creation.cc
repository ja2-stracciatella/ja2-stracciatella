...
static GUIButtonRef MakeTextButton(const ST::string& text, UINT32 const fore_colour, INT16 const x, INT16 const y, INT16 const w, INT16 const h, GUI_CALLBACK const click)
{
	return CreateTextButton(text, SMALLCOMPFONT, fore_colour, FONT_BLACK, x, TASKBAR_Y + y, w, h, MSYS_PRIORITY_NORMAL, click);
}


static void MakeButtonEquipment(UINT idx, UINT level, UINT32 colour, const ST::string& text)
{
...
static void MakeButtonAttribute(UINT idx, UINT level, UINT32 colour, const ST::string& text)
{
