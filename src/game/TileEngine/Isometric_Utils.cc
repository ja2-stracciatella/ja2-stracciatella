...
{
	static GridNo oldGridNo = NOWHERE;
	static UINT32 oldFrameNumber = 99999;

	// if this is the same frame as before, return old value
	if(oldFrameNumber == guiGameCycleCounter && !guiForceRefreshMousePositionCalculation)
		return oldGridNo;

	oldFrameNumber = guiGameCycleCounter;
	guiForceRefreshMousePositionCalculation = FALSE;

	INT16 sWorldX, sWorldY;
	oldGridNo = GetMouseXY(&sWorldX, &sWorldY) ? MAPROWCOLTOPOS_NOCHECK(sWorldY, sWorldX) : NOWHERE;
	return oldGridNo;
}
