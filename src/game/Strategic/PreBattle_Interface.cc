...
void RenderPreBattleInterface()
{
	...
	if (gfRenderPBInterface)
	{
		...
		{
			const INT32 y = BOTTOM_Y - ACTUAL_HEIGHT - ROW_HEIGHT * MAX(guiNumUninvolved, 1);
		...
		mprintf(STD_SCREEN_X + g_ui.m_stdScreenScale * 70, STD_SCREEN_Y + g_ui.m_stdScreenScale * 17, L"%ls %ls", gpStrategicString[STR_PB_SECTOR], sector_name);
