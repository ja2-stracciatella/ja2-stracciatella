...
void RenderAIMMembers()
{
	...
	uiPosX = AIM_MEMBER_OPTIONAL_GEAR_X + StringPixLength( CharacterInfo[AIM_MEMBER_OPTIONAL_GEAR], AIM_M_FONT_STATIC_TEXT) + 5 * g_ui.m_stdScreenScale;
	DrawTextToScreen(wTemp, uiPosX, AIM_MEMBER_OPTIONAL_GEAR_Y, 0, AIM_M_FONT_STATIC_TEXT, AIM_M_COLOR_DYNAMIC_TEXT, FONT_MCOLOR_TRANSPARENT, LEFT_JUSTIFIED);
...
static void DisplayMercsInventory(MERCPROFILESTRUCT const& p)
{
	...
	for (UINT8 i = 0; i < NUM_INV_SLOTS; ++i)
	{
		...
		if (gamepolicy(f_draw_item_shadow))
		{
			...
			BltVideoObjectOutlineShadow(FRAME_BUFFER, &item_vo, item->getGraphicNum(), sCenX - g_ui.m_stdScreenScale * 2, sCenY + g_ui.m_stdScreenScale * 2);
		}
		...
		if (p.bInvNumber[i] > 1)
		{
			...
			DrawTextToScreen(buf, x - 1 * g_ui.m_stdScreenScale, y + 20 * g_ui.m_stdScreenScale,
					 AIM_MEMBER_WEAPON_NAME_WIDTH, AIM_M_FONT_DYNAMIC_TEXT, AIM_M_WEAPON_TEXT_COLOR, FONT_MCOLOR_TRANSPARENT, RIGHT_JUSTIFIED);
		}
...
static void DisplayDots(UINT16 usNameX, UINT16 usNameY, UINT16 usStatX, const wchar_t* pString)
{
	...
	{
		DrawTextToScreen(L".", usPosX, usNameY, 0, AIM_M_FONT_STATIC_TEXT, AIM_M_COLOR_STATIC_TEXT, FONT_MCOLOR_TRANSPARENT, LEFT_JUSTIFIED);
	}
}
...
static BOOLEAN DisplayAimPopUpBox()
{
	...
	if (sPopUpString1[0] != L'\0') y += DisplayWrappedString(usPopUpBoxPosX, y,
	...
	if (sPopUpString2[0] != L'\0')      DisplayWrappedString(usPopUpBoxPosX, y + 4 * g_ui.m_stdScreenScale,
								 AIM_POPUP_BOX_WIDTH, 2 * g_ui.m_stdScreenScale,
