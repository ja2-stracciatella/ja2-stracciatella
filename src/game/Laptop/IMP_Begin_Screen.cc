...
void EnterIMPBeginScreen( void )
{
	...
	// A further I.M.P. has to be of the gender that is still free, otherwise it
	// would take the profile slot of an existing one. With nothing left to pick,
	// make the choice for the player instead of letting them click a dead box.
...
static void BtnIMPBeginScreenDoneCallback(GUI_BUTTON *btn, UINT32 reason)
{
	if (reason & MSYS_CALLBACK_REASON_POINTER_UP)
	{
		...
		else if (GCM->getGamePolicy()->imp_load_saved_merc_by_nickname && IMPSavedProfileDoesFileExist(pNickNameString))
		{
			...
			// up front that no I.M.P. of that gender is occupying the slot already.
			int const iSavedVoiceId = IMPSavedProfileReadVoiceId(pNickNameString);
			if (iSavedVoiceId < 0 ||
				!CanCreateIMPCharacterOfGender(GetIMPGenderFlag(iSavedVoiceId) == IMP_GENDER_MALE))
			{
