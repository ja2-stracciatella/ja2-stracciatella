...
static void GetToolModifier(SKILLCHECKINFO& inf, TOOLINFO& reqTool, SOLDIERTYPE* const soldier)
{
...
static ST::string GetChanceOfSuccessComment(SKILLCHECKINFO& inf, TOOLINFO& reqTool, const int8_t chance)
{
...
static ST::string GetModifiersForSkillAttempts(SKILLCHECKINFO& inf)
{
	ST::string result;
...
static ST::string GetModifiersForLockExam(SOLDIERTYPE* const s, DOOR* const d)
{
	...
	auto getRangeStr = [](int32_t lBound, int32_t uBound) {
		ST::string result{};
		if (lBound != uBound) {
...
static ST::string GetModifiersForLockUntrap(SOLDIERTYPE* const s, DOOR* const d)
{
	SKILLCHECKINFO skillCheckInf{};
...
static ST::string GetModifiersForLockForceOpen(SOLDIERTYPE* const s, DOOR* const d, uint32_t buttonName)
{
	...
	int32_t fatigueMod;
	bool const isCrowbarUsed = buttonName == USE_CROWBAR_ICON;
	if (isCrowbarUsed) {
		requiredTool.name = CROWBAR;
		requiredTool.mustBeUsable = true;
		GetToolModifier(skillCheckInf, requiredTool, s);
...
static ST::string GetModifiersForLockPicking(SOLDIERTYPE* const s, DOOR* const d)
{
	LOCK* const l = &LockTable[d->ubLockID];
	SKILLCHECKINFO skillCheckInf{};
	...
	if (!gOpenDoorMenu.fClosingDoor && gamepolicy(informative_popups)) {
		...
		switch (idx) {
			...
			case USE_CROWBAR_ICON:
				revealedMods = GetModifiersForLockForceOpen(soldier, door, idx);
				break;
		}
	}
