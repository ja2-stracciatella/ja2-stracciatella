...
#define ITEMDESC_START_X				(g_ui.m_stdScreenScale * (INTERFACE_START_X + 214))
...
#define KEYRING_X      (INTERFACE_START_X + g_ui.m_stdScreenScale * 496)
...
void InitInvSlotInterface(INV_REGION_DESC const* const pRegionDesc, INV_REGION_DESC const* const pCamoRegion, MOUSE_CALLBACK const INVMoveCallback, MOUSE_CALLBACK const INVClickCallback, MOUSE_CALLBACK const INVMoveCamoCallback, MOUSE_CALLBACK const INVClickCamoCallback)
{
	...
	for (INT32 i = 0; i != NUM_INV_SLOTS; ++i)
	{
		...
		MSYS_DefineRegion(&m, x, y,
			std::min(x + INT16(g_ui.m_stdScreenScale * r.w), g_ui.m_screenWidth - 1),
...
void InternalInitItemDescriptionBox(OBJECTTYPE *o, const INT16 sX, const INT16 sY, const UINT8 ubStatusIndex, SOLDIERTYPE *s)
{
	...
	if (in_map)
	{
		...
		giMapInvDescButton = QuickCreateButtonImg(INTERFACEDIR "/itemdescdonebutton.sti", 0, 1,
			...
			MSYS_PRIORITY_HIGHEST, ButtonCallbackPrimarySecondary(ItemDescDoneButtonCallbackPrimary, ItemDescDoneButtonCallbackSecondary));
	...
	{
		INT16         const pros_cons_indent = std::max(StringPixLength(gzProsLabel, ITEMDESC_FONT), StringPixLength(gzConsLabel, ITEMDESC_FONT)) + g_ui.m_stdScreenScale * 10;
...
void RenderItemDescriptionBox(void)
{
	...
	if (item->isWeapon())
	{
		...
		//LABELS
		MPrint(dx + g_ui.m_stdScreenScale * ids[0].sX, dy + g_ui.m_stdScreenScale * ids[0].sY, st_format_printf(gWeaponStatsDesc[0], GetWeightUnitString())); // mass
	...
	else
	{
		...
		//Weight
		MPrint(dx + g_ui.m_stdScreenScale * ids[0].sX, dy + g_ui.m_stdScreenScale * ids[0].sY, st_format_printf(gWeaponStatsDesc[0], GetWeightUnitString()));
