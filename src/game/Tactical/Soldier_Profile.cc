...
void GiveProfileToMiltiaSoldier(SOLDIERTYPE* const pSoldier, ProfileID const pid)
{
	...
	MERCPROFILESTRUCT& profile = GetProfile(pid);

	if (pSoldier->ubBodyType == REGFEMALE )
	{
		memcpy(&profile, &GetProfile(mercprofileID_template_female), sizeof(MERCPROFILESTRUCT));
		wcscpy(profile.zName, L"Mrs. Smith");
	}
	else
	{
		memcpy(&profile, &GetProfile(mercprofileID_template_male), sizeof(MERCPROFILESTRUCT));
		wcscpy(profile.zName, L"Mr. Smith");
	}

	switch (pSoldier->ubSoldierClass)
	{
		case SOLDIER_CLASS_GREEN_MILITIA:
			wcscpy(profile.zNickname, L"Boot");
			break;
		case SOLDIER_CLASS_REG_MILITIA:
			wcscpy(profile.zNickname, L"Regular");
			break;
		case SOLDIER_CLASS_ELITE_MILITIA:
			wcscpy(profile.zNickname, L"Veteran");
			break;
		default:
			wcscpy(profile.zNickname, L"Militia");
			break;
	}
	...
	SET_PALETTEREP_ID( profile.HAIR,  pSoldier->HeadPal );
	SET_PALETTEREP_ID( profile.SKIN,  pSoldier->SkinPal );
	SET_PALETTEREP_ID( profile.VEST,  pSoldier->VestPal );
	SET_PALETTEREP_ID( profile.PANTS, pSoldier->PantsPal);
