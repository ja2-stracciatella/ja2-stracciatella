...
static UINT8 MinAPsToPunch(SOLDIERTYPE const& s, GridNo gridno, bool const add_turning_cost)
{
	...
	{ // Is it the same as he's facing?
		...
		if(direction != s.bDirection)
		{
			...
			if (gAnimControl[ s.usAnimState ].ubEndHeight == ANIM_STAND && s.sWalkToAttackWalkToCost == AP_MOVE_STANDING)
			{
				ap += AP_LOOK_STANDING; // ATE: Use standing turn cost
			}
			else if(gAnimControl[ s.usAnimState ].ubEndHeight == ANIM_CROUCH && s.sWalkToAttackWalkToCost == AP_MOVE_CROUCHING)
			{
				ap += AP_LOOK_CROUCHED;
			}
			else if(gAnimControl[ s.usAnimState ].ubEndHeight == ANIM_PRONE && s.sWalkToAttackWalkToCost == AP_MOVE_PRONING)
			{
				ap += AP_LOOK_PRONE;
			} 
		}
