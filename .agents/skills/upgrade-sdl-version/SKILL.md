...
Also update the `*.java` files in `android/app/src/main/java/org/libsdl/app` with their release versions. They can be found in the SDL repository at `android-project/app/src/main/java/org/libsdl/app`. Ensure you use the release tag to update those files. There should be no project specific changes to those files.
