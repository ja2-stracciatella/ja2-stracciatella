...
impl LoggedGuess {
    fn guess_vanilla_version(&mut self, gamedir: &Path) -> GuessResult<()> {
        ...
        for path in self.get_pack_paths()? {
            match self.compare_pack(&datadir, &path) {
                Ok((version, extras)) => {
                    if extras.len() == 0 {
                        info!("Found perfect match with vanilla_version {:?}\n", version);
                        self.vanilla_version = Some(version);
                        return Ok(());
                    }
                    info!("Found match with vanilla_version {:?}\n", version);
        ...
        if best_version.is_some() {
            ...
            info!("Using best match with vanilla_version {:?}\n", version);
    ...
    fn get_pack_paths(&mut self) -> GuessResult<Vec<PathBuf>> {
        ...
        info!("Searching for resource packs in {:?}\n", &dir);
    ...
    fn match_resources(
        ...
        info!("Found {} resources (no hashes)\n", resources.len());
        ...
        if hashes.len() == 0 {
            ...
            info!("Passed (no hashes) with {} extra resources\n", extras.len());
            return Ok(extras);
        }
        ...
        info!("Passed with {} extra resources\n", extras.len());
        return Ok(extras);
    }
}
