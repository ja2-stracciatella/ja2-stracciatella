...
void RenderFlorist()
{
  HVOBJECT hPixHandle;
	...
	{
		BltVideoObject(FRAME_BUFFER, hPixHandle, 0, FLORIST_FIRST_BULLET_X, usPosY, VO_BLT_SRCTRANSPARENCY);
...
void DisplayFloristDefaults()
{
  HVOBJECT hPixHandle;
	...
	if( guiCurrentLaptopMode == LAPTOP_MODE_FLORIST )
	{
		...
		BltVideoObject(FRAME_BUFFER, hPixHandle, 0,FLORIST_BIG_TITLE_X, FLORIST_BIG_TITLE_Y, VO_BLT_SRCTRANSPARENCY);
	}
	else
	{
		gfHomePageActive = FALSE;
		GetVideoObject(&hPixHandle, guiSmallTitleSymbol);
		BltVideoObject(FRAME_BUFFER, hPixHandle, 0,FLORIST_SMALL_TITLE_X, FLORIST_SMALL_TITLE_Y, VO_BLT_SRCTRANSPARENCY);
	}
}
