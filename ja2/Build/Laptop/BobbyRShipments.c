...
void DisplayShipmentGrid()
{
  HVOBJECT hPixHandle;
	...
	// Shipment Order Grid
  BltVideoObject(FRAME_BUFFER, hPixHandle, 0, BOBBYR_SHIPMENT_DELIVERY_GRID_X, BOBBYR_SHIPMENT_DELIVERY_GRID_Y, VO_BLT_SRCTRANSPARENCY);

	// Order Grid
  BltVideoObject(FRAME_BUFFER, hPixHandle, 1, BOBBYR_SHIPMENT_ORDER_GRID_X, BOBBYR_SHIPMENT_ORDER_GRID_Y, VO_BLT_SRCTRANSPARENCY);
}
...
void DisplayPreviousShipments()
{
	...
	for( uiCnt=0; uiCnt<uiNumItems; uiCnt++ )
	{
				...
				gpNewBobbyrShipments[ giBobbyRShipmentSelectedShipment ].fDisplayedInShipmentPage )
		{
			...
			swprintf( zText, lengthof(zText), L"%S %d", gpGameClockString[0], gpNewBobbyrShipments[ uiCnt ].uiOrderedOnDayNum );
